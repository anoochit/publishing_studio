...
An API Gateway must efficiently direct inbound client requests to the correct backend services while protecting those services from overwhelming traffic. In an era of polyglot microservices, legacy monoliths, and specialized AI backends, traffic routing and management remain critical.
...
### Discussion
Kong evaluates routes based on a specific priority sequence. By specifying both `paths` and `headers`, you create a logical `AND` condition. In this recipe, Kong routes a request to `/api/billing` to the `v1` backend only if the request includes the `x-api-version: 1.0` header. The `strip_path: true` directive ensures Kong removes `/api/billing` from the URI before forwarding the request to the upstream service.
...
### Discussion
By setting the `host` field of the Service to the name of the `Upstream`, Kong bypasses external DNS resolution and utilizes its internal ring-balancer. In this configuration, Kong distributes traffic using a `round-robin` algorithm. The `weight` parameter allows for proportional distribution; here, `10.0.1.12` receives half as much traffic as the other two nodes. You can also utilize `least-connections` or `consistent-hashing` algorithms based on your use case.
...
### Discussion
This recipe restricts a single IP address to 60 requests per minute and 1,000 requests per hour. The `policy` defines where Kong stores the counter data. The `local` policy stores data in the memory of the individual Kong node (useful for single nodes or sticky-session setups). In a robust production environment with multiple Kong nodes, change the `policy` to `redis` and provide your Redis host details to track limits cluster-wide.
...
### Discussion
The `proxy-cache` plugin provides a massive performance boost by storing the responses of upstream requests. In this configuration, Kong caches successful JSON responses in memory for 5 minutes (300 seconds). Kong serves any subsequent request to the identical URI within that window directly, dropping response times from hundreds of milliseconds to under 2 milliseconds, effectively shielding the legacy system from heavy read traffic.
